...
def main(args):
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            checkpoint = torch.load(args.resume) #map_location = torch.device('cpu')) # For CPU
    ...
    with torch.set_grad_enabled(grad_flag):
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            if attacker is not None:
                ...
                print("Adv img predicted", torch.argmax(output, dim=1))
                print("Adv img prob", torch.max(output, dim=1))
                print("Orig img predicted", torch.argmax(orig_output, dim=1))
                print("Orig img prob", torch.max(orig_output, dim=1))
            else:
                # forward the model
                output = model(input)
